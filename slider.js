...
class ItcSlider {
    ...
    #autoplay(action) {
        if (!this.#config.autoplay || this.isLessThanRequired) {
            return;
        }
        if (action === 'stop') {
    ...
    #init() {
        ...
        this.isLessThanRequired = this.#state.elListItem.length <= this.#state.countActiveItems;
        if (this.isLessThanRequired && this.#state.btnNext) {
            this.#state.btnNext.classList.add(this.#state.btnClassHide);
        }
        if (this.#config.loop && !this.isLessThanRequired) {
